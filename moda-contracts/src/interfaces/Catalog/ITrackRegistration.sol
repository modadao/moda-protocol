...
interface ITrackRegistration {
     ...
     * @notice `PENDING` A track was registered, but is pending approval. This will be the inital state for anyone registering without the `GOLD_ROLE`.
     * @notice `VALIDATED` After a track has been submitted and is approved by a privileged account it will be `VALIDATED`.
     * @notice `INVALIDATED` If a track submission is fraudulous or invalid and has been rejected by a privileged account it will be `INVALIDATED`.
     */
    enum TrackStatus {
        PENDING,
        VALIDATED,
