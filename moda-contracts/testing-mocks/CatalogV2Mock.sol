...
contract CatalogV2Mock is ICatalog, AccessControlUpgradeable {
    ...
    struct CatalogStorage {
        ...
        /// @dev Name of the product the registry is used for, denoted as <Name->
        string _catalogName;
        /// @dev The chain id of the network the registry is deployed on, denoted as <ChainId->
        string _chainId;
        /// @dev The version of the registry, denoted as <Version->
    ...
    function _requireUserIsMember(uint256 catalogIndex, address user) internal {
        ...
        if (IModaRegistry($._modaRegistry).isMember(catalogIndex, user) == false) {
    ...
    function _requireUserIsArtistOrManager(address user, address artist) internal view {
        ...
        if (user != artist && IModaRegistry($._modaRegistry).isManager(artist, user) == false) {
    ...
    function _requireUserHasVerifierRole(address user) internal view {
        ...
        if (IModaRegistry($._modaRegistry).hasRole(keccak256("VERIFIER_ROLE"), user) == false) {
    ...
    function _requireCallerHasReleasesRegistrarRole(address caller) internal view {
        CatalogStorage storage $ = _getCatalogStorage();
        if (
            IModaRegistry($._modaRegistry).hasRole(keccak256("RELEASES_REGISTRAR_ROLE"), caller) == false
        ) {
    ...
    function _requireReleasesContractHasPermission(string calldata trackId) internal view {
        ...
        if ($._singleTrackReleasesPermission[trackId][msg.sender] == false) {
    ...
    function _createReleaseHash(
        ...
        return keccak256(abi.encodePacked(packedHashes, uri));
    }
    ...
    function _packStringArray(string[] memory array) internal pure returns (bytes memory) {
        ...
        for (uint256 i = 0; i < array.length; i++) {
            packedBytes = abi.encodePacked(packedBytes, array[i]);
        }
        return packedBytes;
    }
}
